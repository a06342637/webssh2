...
- SFTP 文件新建、在线编辑、删除、流式上传，以及带进度的文件/文件夹下载
- 脚本名称/命令片段即时搜索
...
在 SFTP 列表中可以在当前目录新建文件，或打开现有 UTF-8 文本文件（包括可访问的符号链接目标）进行在线编辑。编辑器支持保存、最小化、最大化和未保存关闭确认；服务端使用文件版本校验和目标路径校验，防止无声覆盖已被其他人修改或重新指向的内容。普通文件可在二次确认后删除，删除符号链接时只删除链接本身。
