...
- SSH 与 RDP 双向剪贴板，浏览器拒绝剪贴板权限时自动降级到手动粘贴框
...
- **远端 → 本地**：远端复制后内容先缓存在标签页里。若浏览器允许自动写入就直接落盘；被拒绝时工具栏的复制按钮会亮起一个小圆点，点它（或按 `Ctrl+Shift+C`）即可取回
- **本地 → 远端**：点粘贴按钮或按 `Ctrl+Shift+V` 把本地剪贴板推给远端，然后在远程桌面里正常按 `Ctrl+V`；点回远程桌面画面时也会顺带同步一次
- 浏览器完全禁止读取剪贴板（例如站点是纯 HTTP）时，会弹出一个手动粘贴框，在里面按 `Ctrl+V` 即可，多行内容不会被压成一行

想要完全无感的自动同步，请给站点配置 HTTPS。
