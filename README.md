...
- 一键分享连接：生成明文或端到端加密的直连链接，对方打开即自动连上同一台服务器
- SSH 与 RDP 双向剪贴板，浏览器拒绝剪贴板权限时自动降级到手动粘贴框；复制键可在 Ctrl 与 ⌘ 之间切换，Mac 上 ⌘ 会当作 Ctrl 发给远端
...
连上 SSH 或 RDP 后，点终端顶栏右侧的分享按钮，可以把当前这台服务器生成一条直连链接，对方打开即自动连上。SSH 和 RDP 都支持。

### 明文分享

凭据 Base64 编码后直接挂在链接的 `#` 之后，**完全不经过服务器**：

```text
https://你的域名/#ssh=<base64url-json>     # SSH，与上面的 URL 快速登录同格式
https://你的域名/#rdp=<base64url-json>     # RDP
```

Fragment 不会随 HTTP 请求发给服务端，也不进访问日志，但它会留在浏览器历史、书签、聊天记录和截图里。**谁拿到链接谁就能登录**，只发给你信任的人。

分享时会自动剔除 `trustScope`（本机的 SSH 主机密钥信任域），接收方按自己的 TOFU 策略重新确认主机密钥。
...
```text
https://你的域名/s/<token>#k=<密钥>
```

- 凭据在**你的浏览器里**用 AES-GCM 256 加密，只有密文和 IV 上传到服务端
- 解密密钥放在 `#` 之后。浏览器按规范不会把 Fragment 发给服务器，所以**服务端全程只见密文**，即使数据库整个泄漏也解不出任何密码
- 数据库里存的是 token 的 SHA-256 哈希，不是可以直接使用的 token 本身
- 可选有效期 15 分钟 / 1 小时 / 24 小时 / 7 天，以及「阅后即焚」（链接只能成功打开一次）
- 创建需要登录 WebSSH 书签账号（否则接口会沦为任何人可用的免费加密存储）；**读取不需要登录**，因为接收方通常没有本站账号

隐私分享依赖浏览器的 `crypto.subtle`，它只在 HTTPS（或 localhost）下可用。站点跑在纯 HTTP 时该选项会自动禁用并提示改用明文分享，不会假装加密。

注意：加密保护的是「链接在传输和存储过程中不被第三方还原」。**拿到完整链接的人依然能连上这台服务器**——密钥就在链接里。
...
RDP 剪贴板受浏览器安全策略硬性限制：`navigator.clipboard` 只在 **HTTPS（或 localhost）** 且**有用户手势**时才放行。而 RDP 的两个剪贴板事件都是远端触发的，天然没有手势，浏览器会直接拒绝。

因此双向同步是这样工作的：

- **远端 → 本地**：远端复制后内容先缓存在标签页里。若浏览器允许自动写入就直接落盘；被拒绝时工具栏的复制按钮会亮起一个小圆点，点它（或按复制快捷键）即可取回
- **本地 → 远端**：点粘贴按钮或按粘贴快捷键把本地剪贴板推给远端，然后在远程桌面里正常粘贴；点回远程桌面画面时也会顺带同步一次
- 浏览器完全禁止读取剪贴板（例如站点是纯 HTTP）时，会弹出一个手动粘贴框，在里面粘贴即可，多行内容不会被压成一行

自动同步不会把刚从远端收到的内容再推回远端，避免来回覆盖。想要完全无感的自动同步，请给站点配置 HTTPS。

### Mac 用户：复制键是 ⌘ 还是 Ctrl

Mac 的键盘习惯和远端 Windows 是冲突的：本地用 **⌘ 复制**，远端只认 **Ctrl**。而浏览器把 ⌘ 报成 `MetaLeft/MetaRight`，原样发过去就是 **Windows 键**——在远端按 ⌘+C 弹出的是开始菜单，不是复制。

「远程桌面设置 → 输入与剪贴板 → 复制 / 粘贴修饰键」可以选：

| 选项 | WebSSH 快捷键 | 按下 ⌘ 时发给远端的键 |
|---|---|---|
| **自动**（默认） | Mac 上是 ⌘+Shift+C/V，其它平台是 Ctrl+Shift+C/V | Mac 上发 Ctrl，其它平台发 Win |
| **Ctrl** | Ctrl+Shift+C/V | Win 键（原样） |
| **Command ⌘ / Win** | ⌘+Shift+C/V | Ctrl |

选 Command 时 ⌘ 会被当作 Ctrl 发给远端，所以在远程桌面里直接按 **⌘+C / ⌘+V** 就能复制粘贴，和 Mac 本地习惯一致。如果你反而需要用到远端的 Windows 键（比如按 Win+E 开资源管理器），把这一项改成 **Ctrl**。

工具栏按钮的悬停提示会跟着这个设置显示实际该按的键。
